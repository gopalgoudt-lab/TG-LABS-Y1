...
type PartnerOffer = { id:string; price:number; availability:'AVAILABLE'|'CHECK_AVAILABILITY'|'UNAVAILABLE'; tat?:string|null; partner:{ id:string; slug:string; name:string } };
type CatalogTest = { id:string; slug:string; name:string; description?:string|null; mrp:number; price:number; diagnosticPartner?:string|null; tat?:string|null; fastingNeeded:boolean; sampleTypes:string[]; offers:PartnerOffer[] };
...
const partners:Partner[] = [
  { name:'Sagepath Labs', logo:'/partners/sagepath-labs.svg', description:'Regional speciality testing', reports:'Check availability', startingPrice:'Check availability', specialty:'Regional specialty testing' },
  { name:'Thyrocare', logo:'/partners/thyrocare.svg', description:'Preventive profiles & packages', reports:'Check availability', startingPrice:'Check availability', specialty:'Preventive profiles & packages' },
  { name:'Dr Lal PathLabs', logo:'/partners/dr-lal-pathlabs.svg', description:'Routine & specialised diagnostics', reports:'Check availability', startingPrice:'Check availability', specialty:'Routine & specialised diagnostics' },
];
const displayedPartnerNames = ['TG Labs partner', ...partners.map((partner)=>partner.name)];
...
export default function Home(){
  ...
  useEffect(()=>{ const saved=window.localStorage.getItem('tglabs-cart'); if(!saved)return; try{ const parsed=JSON.parse(saved); if(Array.isArray(parsed)) setCart(parsed.filter((item):item is CartItem=>Boolean(item&&typeof item==='object'&&(item.kind==='package'||(item.kind==='test'&&item.offerId&&item.partnerId&&item.partnerName))))); }catch{ window.localStorage.removeItem('tglabs-cart'); } },[]);
  useEffect(()=>{ window.localStorage.setItem('tglabs-cart',JSON.stringify(cart)); },[cart]);

  const filteredTests=useMemo(()=>{ const term=query.trim().toLowerCase(); if(!term)return tests; return tests.filter(t=>`${t.name} ${t.description||''} ${t.offers.map(offer=>offer.partner.name).join(' ')}`.toLowerCase().includes(term)); },[query,tests]);
  ...
  const addToCart=(item:CartItem)=>{ setCart(current=>[...current.filter(x=>!(x.kind===item.kind&&x.id===item.id)),item]); setShowCart(true); };
  ...
  const filterPartnerNames=useMemo(()=>Array.from(new Set([...displayedPartnerNames,...tests.flatMap(test=>test.offers.map(offer=>offer.partner.name))])),[tests]);
  const featuredTests=(query?filteredTests:tests).filter(test=>partnerFilter==='all'||test.offers.some(offer=>offer.availability==='AVAILABLE'&&offer.partner.name===partnerFilter)).slice(0,4);
  ...
  return <>
    ...
    <main>
      ...
      <section id="tests" className="compareSection"><div className="marketWrap"><div className="sectionTop"><div><span className="kicker">COMPARE BEFORE YOU BOOK</span><h2>{query||'Popular diagnostic tests'}</h2><p>Choose a test and review the performing diagnostic partner before adding it.</p></div><label className="partnerFilterLabel">Filter by laboratory<select className="partnerSelect" value={partnerFilter} onChange={e=>setPartnerFilter(e.target.value)}><option value="all">All partners</option>{filterPartnerNames.map(name=><option key={name} value={name}>{name}</option>)}</select></label></div>{catalogError&&<div className="notice" role="alert">{catalogError}</div>}{catalogLoading&&<div className="catalogStatus" role="status">Loading current tests and prices…</div>}<div className="comparisonList">{featuredTests.map(test=>{const availableOffers=test.offers.filter(offer=>offer.availability==='AVAILABLE');const selectedOffer=availableOffers.find(offer=>offer.id===selectedOfferIds[test.id])||availableOffers[0];const offerByPartner=new Map(test.offers.map(offer=>[offer.partner.name,offer]));const optionNames=Array.from(new Set([...test.offers.map(offer=>offer.partner.name),'Sagepath Labs','Thyrocare']));return <article className="comparisonCard" key={test.id}><div className="testInfo"><h3>{test.name}</h3><p>{test.sampleTypes?.[0]||'Blood sample'} • Home collection available</p><span className="detailNote">Preparation confirmed during booking</span></div><div className="partnerChoices">{optionNames.map(name=>{const offer=offerByPartner.get(name);const bookable=Boolean(offer&&offer.availability==='AVAILABLE');const selected=bookable&&offer?.id===selectedOffer?.id;return <button type="button" className={`choice ${selected?'active':''} ${bookable?'':'mutedChoice'}`} key={name} disabled={!bookable} onClick={()=>offer&&setSelectedOfferIds(current=>({...current,[test.id]:offer.id}))} aria-pressed={selected}><span className="radio" aria-hidden="true">{selected?'●':'○'}</span><div><b>{name}</b><small>{bookable?`✓ Verified partner • ${offer?.tat||'TAT confirmed before booking'}`:'Availability confirmed before payment'}</small></div><strong>{bookable?`₹${offer!.price.toLocaleString('en-IN')}`:'Check availability'}</strong>{selected&&<em>SELECTED</em>}</button>})}</div><button type="button" className="addBlue" disabled={!selectedOffer} onClick={()=>selectedOffer&&addToCart({kind:'test',id:test.id,name:test.name,price:selectedOffer.price,offerId:selectedOffer.id,partnerId:selectedOffer.partner.id,partnerName:selectedOffer.partner.name,tat:selectedOffer.tat})}>{selectedOffer?inCart('test',test.id)?'Update cart ✓':'Add to cart':'Check availability'}</button></article>})}</div>{!catalogLoading&&!catalogError&&featuredTests.length===0&&<div className="catalogStatus">No tests match this search and laboratory filter. Try another partner or search term.</div>}</div></section>
