...
type Test = [string, string, string, number];
type Package = [string, number, string];
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    if (saved) {
      try { setCart(JSON.parse(saved)); } catch { window.localStorage.removeItem('tglabs-cart'); }
    }
  }, []);
  ...
  const results = useMemo(() => {
    ...
    return tests.filter(([cat, name, desc]) => `${cat} ${name} ${desc}`.toLowerCase().includes(term));
  }, [query]);
  ...
  return (
    <>
      ...
      {showCart && <div className="cartOverlay" role="dialog" aria-modal="true" aria-label="Your cart"><div className="cartPanel"><div className="cartHeader"><div><small>YOUR BASKET</small><h2>Ready to book?</h2></div><button className="iconBtn" type="button" onClick={() => setShowCart(false)} aria-label="Close cart">×</button></div>{cart.length === 0 ? <div className="emptyCart">Your basket is empty. Add a test to start your booking.</div> : <><div className="cartItems">{cart.map((name) => { const test = tests.find((item) => item[1] === name); return <div className="cartItem" key={name}><div><b>{name}</b><span>Diagnostic test</span></div><div><b>₹{test?.[3].toLocaleString('en-IN')}</b><button type="button" onClick={() => removeFromCart(name)}>Remove</button></div></div>; })}</div><a className="btn primary full" href="/checkout" onClick={() => setShowCart(false)}>Continue to booking →</a></>}</div></div>}
      ...
      <main>
        ...
        <section id="tests"><div className="wrap"><div className="head rowHead"><div><small>POPULAR TESTS</small><h2>Start with the tests you need.</h2><p>Search, compare and add tests to your basket.</p></div><span className="resultCount">{results.length} tests</span></div><div className="grid">{results.map(([cat, name, desc, price]) => <article className="card testCard" key={name}><span className="pill">{cat}</span><h3>{name}</h3><p>{desc}</p><div className="cardBottom"><div className="price">₹{price.toLocaleString('en-IN')}</div><button className="btn primary" type="button" onClick={() => addToCart(name)}>{cart.includes(name) ? 'Added ✓' : 'Add to Cart'}</button></div></article>)}</div>{results.length === 0 && <div className="notice">No matching test found. Try HbA1c, CBC, Vitamin D or Thyroid.</div>}</div></section>

        <section id="packages" className="dark"><div className="wrap"><div className="head"><small>HEALTH PACKAGES</small><h2>Preventive care, packaged.</h2><p>Explore practical health-check options for routine preventive screening.</p></div><div className="grid">{packages.map(([name, price, desc]) => <article className="card packageCard" key={name}><span className="packageTag">HEALTH PACKAGE</span><h3>{name}</h3><p>{desc}</p><div className="price">₹{price.toLocaleString('en-IN')}</div><a className="btn primary" href="#booking">View Package →</a></article>)}</div></div></section>
