...
const FLOW=[['BOOKING_CREATED','Booking created'],['BOOKING_CONFIRMED','Confirmed'],['TECHNICIAN_ASSIGNED','Technician assigned'],['TECHNICIAN_ACCEPTED','Accepted'],['ON_THE_WAY','On the way'],['REACHED_PATIENT','Reached'],['SAMPLE_COLLECTED','Sample collected'],['SAMPLE_RECEIVED_AT_LAB','At lab'],['PROCESSING','Processing'],['REPORT_READY','Report ready'],['REPORT_DELIVERED','Delivered']] as const;

type Booking={id:string;orderNumber:string;status:string;paymentStatus:string;workflowStatus:string;total:number;mode:string;collectionDate:string;slot:string;tests:string[];packages:string[];doctorName?:string|null;printedReport?:boolean;printedReportFee?:number;technician?:string|null;timeline:Record<string,string|null>};
...
export default function PatientPage(){
 ...
 const[phone,setPhone]=useState(''),[name,setName]=useState(''),[orders,setOrders]=useState<Booking[]>([]),[reports,setReports]=useState<Report[]>([]),[loading,setLoading]=useState(true),[error,setError]=useState('');
 const[aiReports,setAiReports]=useState<Record<string,AiReport>>({}),[aiBusy,setAiBusy]=useState(''),[aiError,setAiError]=useState<Record<string,string>>({});
 useEffect(()=>onAuthStateChanged(auth,async user=>{if(!user){router.replace('/auth');return}setPhone(user.phoneNumber??'');setLoading(true);setError('');try{const token=await user.getIdToken(),headers={Authorization:`Bearer ${token}`};const[br,rr]=await Promise.all([fetch('/api/patient/bookings',{headers,cache:'no-store'}),fetch('/api/patient/reports',{headers,cache:'no-store'})]);if(br.status===401||rr.status===401){await signOut(auth);router.replace('/auth');return}if(!br.ok||!rr.ok)throw new Error('Unable to load your TG Labs account right now. Please try again.');const b=await br.json(),r=await rr.json();setOrders(b.orders??[]);setReports(r.reports??[]);setName(b.patient?.name??'')}catch(e){setError(e instanceof Error?e.message:'Unable to load your account.')}finally{setLoading(false)}}),[router]);
 ...
 async function generateAiReport(report:Report){
  ...
  const consent=window.confirm('AI Report will securely send this diagnostic report to OpenAI for an educational explanation. It is not a diagnosis or substitute for your doctor. Continue?');
  if(!consent)return;
  const user=auth.currentUser;if(!user){router.replace('/auth');return}
  setAiBusy(report.id);setAiError(x=>({...x,[report.id]:''}));
  try{
   const token=await user.getIdToken();
   const r=await fetch(`/api/patient/reports/${report.id}/ai`,{method:'POST',headers:{Authorization:`Bearer ${token}`}}),j=await r.json();
   if(r.status===401){await signOut(auth);router.replace('/auth');return}
   if(!r.ok)throw new Error(j.error||'Unable to generate AI Report.');
   setAiReports(x=>({...x,[report.id]:j}));
  }catch(e){setAiError(x=>({...x,[report.id]:e instanceof Error?e.message:'Unable to generate AI Report.'}))}finally{setAiBusy('')}
 }
 ...
 return <main style={{minHeight:'100vh',background:'#f6faf9',color:'#15312c'}}><div style={{maxWidth:1180,margin:'0 auto',padding:'32px 20px 64px'}}>
  <header style={{display:'flex',gap:20,alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',marginBottom:28}}><div><p style={ey}>TG LABS PATIENT PORTAL</p><h1 style={{margin:'6px 0 4px',fontSize:'clamp(30px,5vw,46px)'}}>My TG Labs</h1><p style={{margin:0,color:'#64748b'}}>{name?`Welcome, ${name}`:'Your secure diagnostic dashboard'} {phone?`• ${phone}`:''}</p></div><div style={{display:'flex',gap:10,flexWrap:'wrap'}}><a href="/tests" style={secondary}>Browse tests</a><a href="/booking" style={primary}>Book new test</a><button onClick={handleSignOut} style={secondary}>Sign out</button></div></header>
  ...
  <section style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(210px,1fr))',gap:16,marginBottom:22}}><Stat label="Total bookings" value={orders.length}/><Stat label="Active" value={upcoming.length}/><Stat label="Reports" value={reports.length}/></section>
  {!loading&&tracking&&<section style={{...panel,marginBottom:22,border:'1px solid #b7ddd4'}}><div style={{display:'flex',justifyContent:'space-between',gap:16,flexWrap:'wrap',alignItems:'start'}}><div><p style={ey}>LIVE BOOKING TRACKER</p><h2 style={{margin:'4px 0 5px'}}>{tracking.orderNumber}</h2><div style={{color:'#64748b'}}>{date(tracking.collectionDate)} • {tracking.slot} • {tracking.mode==='HOME'?'Home collection':'Centre visit'}</div></div><Status text={tracking.workflowStatus}/></div>
  ...
  {loading?<section style={panel}><h2>Loading your account…</h2></section>:<div style={{display:'grid',gap:22}}>
   ...
   <section style={panel}><div style={head}><div><p style={ey}>REPORTS</p><h2 style={{margin:'4px 0 0'}}>Diagnostic reports</h2><p style={{margin:'6px 0 0',color:'#64748b',fontSize:13}}>View the original laboratory report or ask AI for a simpler educational explanation.</p></div></div>{reports.length===0?<Empty title="No reports available yet"/>:<div style={{display:'grid',gap:12}}>{reports.map(r=><div key={r.id} style={{display:'grid',gap:10}}><article style={row}><div><strong>{r.name}</strong><p style={{margin:'6px 0',color:'#64748b',fontSize:14}}>{r.orderNumber} • {[...r.packages,...r.tests].slice(0,3).join(' • ')}</p><Status text={r.status}/>{r.publishedAt&&<div style={{fontSize:12,color:'#64748b',marginTop:6}}>Published {stamp(r.publishedAt)}</div>}</div>{r.downloadUrl?<div style={{display:'flex',gap:8,flexWrap:'wrap',alignItems:'center'}}><a href={r.downloadUrl} target="_blank" rel="noreferrer" style={primary}>View / Download report</a><button type="button" onClick={()=>generateAiReport(r)} disabled={aiBusy===r.id} style={{...aiButton,opacity:aiBusy===r.id?.65:1}}>{aiBusy===r.id?'Analyzing…':'✨ AI Report'}</button></div>:<span style={{color:'#64748b'}}>Processing</span>}</article>
