  ...
  useEffect(()=>{
    ...
    function enhance(){
      ...
      for(const label of labels){
        ...
        const foundDiscountInput=label.querySelector(':scope > input[type="number"]') as HTMLInputElement|null;
        ...
        valueInput.type='number'; valueInput.min='0'; valueInput.step='1'; valueInput.placeholder='0';
        ...
        percentCheckbox.type='checkbox'; percentCheckbox.setAttribute('aria-label','Use percentage discount');
        percentCheckbox.style.cssText='width:18px;height:18px;accent-color:#0d9488;cursor:pointer';
        const percentText=document.createElement('span'); percentText.textContent='%';
        ...
        function renderHint(){
          if(percentMode){
            const gross=grossFor(hiddenDiscountInput); const amount=Math.round(gross*percentValue/100);
            hint.textContent=`${percentValue||0}% of ₹${gross.toLocaleString('en-IN')} = ₹${amount.toLocaleString('en-IN')} discount`;
          }else hint.textContent='Enter any discount amount, or tick % to enter a percentage.';
        }
        function apply(){
          if(percentMode){
            ...
            const amount=Math.round(grossFor(hiddenDiscountInput)*percentValue/100);
        ...
        percentCheckbox.addEventListener('change',()=>{
          ...
          if(percentMode){ percentValue=0; valueInput.value='0'; valueInput.max='100'; }
          else { valueInput.removeAttribute('max'); valueInput.value=String(manualValue||0); }
          apply();
        });
        percentLabel.addEventListener('click',(event)=>{
          if(event.target!==percentCheckbox){ event.preventDefault(); percentCheckbox.click(); }
        });
