...
export default function ThyrocareCatalogPage() {
  ...
  const shown = useMemo(
    () =>
      items.filter((x) => {
        ...
        const type = filter === 'ALL' || x.type === filter;
        return hit && type;
      }),
    [items, q, filter],
  );
  ...
  async function remove(x: Item) {
    ...
    try {
      const r = await fetch('/api/admin/thyrocare/catalog', {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ id: x.id, type: x.type }),
      });
      const d = await r.json();
      if (!r.ok) throw new Error(d.error || 'Unable to delete item.');
      await load();
    } catch (e) {
    ...
    try {
      const r = await fetch('/api/admin/thyrocare/catalog', {
        ...
        body: JSON.stringify({
          ...
          description: x.description || '',
          tat: x.tat || '',
          sampleTypes: x.sampleTypes || [],
        }),
      });
      const d = await r.json();
      if (!r.ok) throw new Error(d.error || 'Unable to restore item.');
      await load();
    } catch (e) {
  ...
  return (
    <main style={{ minHeight: '100vh', background: '#f5f7f9', fontFamily: 'Arial,sans-serif', color: '#1f2937' }}>
      <div style={{ maxWidth: 1380, margin: '0 auto', padding: 22 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 16, flexWrap: 'wrap' }}>
          <div>
            <div style={{ fontSize: 12, fontWeight: 900, color: '#087f6f' }}>THYROCARE • MANUAL CATALOGUE</div>
            <h1 style={{ margin: '5px 0' }}>Tests & Packages</h1>
            <div style={{ color: '#64748b' }}>Separate Thyrocare names and prices. These do not change the TG Labs catalogue.</div>
          </div>
          <div style={{ display: 'flex', gap: 8 }}>
            <a href="/manual" style={btnLight}>← Manual Dashboard</a>
            <a href="/admin" style={btnLight}>Admin</a>
          </div>
        </div>

        {error && <div style={err}>{error}</div>}

        <section style={{ display: 'grid', gridTemplateColumns: '390px minmax(0,1fr)', gap: 18, marginTop: 18 }}>
          <form onSubmit={save} style={card}>
            <h2 style={{ marginTop: 0, color: '#0d5f54' }}>{form.id ? 'Edit' : 'Add'} Test / Package</h2>

            <label style={label}>Type
              <select value={form.type} onChange={(e) => setForm((x) => ({ ...x, type: e.target.value as 'TEST' | 'PACKAGE' }))} style={input}>
            ...
            <label style={label}>Name *
              <input required value={form.name} onChange={(e) => setForm((x) => ({ ...x, name: e.target.value }))} style={input} placeholder="e.g. Thyroid Profile" />
            </label>

            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
              <label style={label}>Selling Price (₹) *
                <input required type="number" min="0" value={form.price} onChange={(e) => setForm((x) => ({ ...x, price: e.target.value }))} style={input} />
              </label>
              <label style={label}>MRP (₹)
                <input type="number" min="0" value={form.mrp} onChange={(e) => setForm((x) => ({ ...x, mrp: e.target.value }))} style={input} />
              </label>
            </div>

            <label style={label}>TAT
              <input value={form.tat} onChange={(e) => setForm((x) => ({ ...x, tat: e.target.value }))} style={input} placeholder="e.g. 24 hours" />
            </label>

            <label style={label}>Sample Type
              <select
                value={selectedSampleType}
                onChange={(e) => {
                  const value = e.target.value;
                  setForm((x) => ({ ...x, sampleTypes: value === 'OTHER' ? 'Other' : value }));
                }}
                style={input}
              >
            ...
            {selectedSampleType === 'OTHER' && (
              <label style={label}>Specify Other Sample Type *
                <input
                  required
                  value={form.sampleTypes === 'Other' ? '' : form.sampleTypes}
                  onChange={(e) => setForm((x) => ({ ...x, sampleTypes: e.target.value }))}
                  style={input}
                  placeholder="e.g. Citrate Plasma, Swab, Stool"
                />
              </label>
            )}

            <label style={label}>Description
              <textarea rows={4} value={form.description} onChange={(e) => setForm((x) => ({ ...x, description: e.target.value }))} style={{ ...input, resize: 'vertical' }} />
            </label>

            <label style={{ ...label, display: 'flex', gap: 8, alignItems: 'center' }}>
              <input type="checkbox" checked={form.fastingNeeded} onChange={(e) => setForm((x) => ({ ...x, fastingNeeded: e.target.checked }))} /> Fasting required
            </label>
            <label style={{ ...label, display: 'flex', gap: 8, alignItems: 'center' }}>
              <input type="checkbox" checked={form.active} onChange={(e) => setForm((x) => ({ ...x, active: e.target.checked }))} /> Active
            </label>

            <div style={{ display: 'flex', gap: 8, marginTop: 15 }}>
              <button disabled={saving} style={btnPrimary}>{saving ? 'Saving…' : form.id ? 'Update' : 'Add Item'}</button>
              <button type="button" onClick={reset} style={btnLight}>Clear</button>
            </div>
          </form>
          ...
          <div style={card}>
            <div style={{ display: 'flex', justifyContent: 'space-between', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
              <div>
                <h2 style={{ margin: '0 0 4px', color: '#0d5f54' }}>Thyrocare Price List</h2>
                <div style={{ fontSize: 12, color: '#64748b' }}>{items.length} items</div>
              </div>
              <div style={{ display: 'flex', gap: 8 }}>
                <input value={q} onChange={(e) => setQ(e.target.value)} style={{ ...input, width: 260, marginTop: 0 }} placeholder="Search test/package" />
                <select value={filter} onChange={(e) => setFilter(e.target.value)} style={{ ...input, width: 130, marginTop: 0 }}>
            ...
            <div style={{ overflowX: 'auto', marginTop: 14 }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 850 }}>
                <thead>
                  <tr>{['Type', 'Name', 'MRP', 'Price', 'TAT', 'Fasting', 'Status', 'Actions'].map((h) => <th key={h} style={th}>{h}</th>)}</tr>
                </thead>
                <tbody>
                  {loading ? <tr><td colSpan={8} style={td}>Loading…</td></tr> : shown.length === 0 ? <tr><td colSpan={8} style={td}>No Thyrocare catalogue items yet.</td></tr> : shown.map((x) => (
                    <tr key={`${x.type}-${x.id}`}>
                      <td style={td}><span style={pill}>{x.type === 'TEST' ? 'Test' : 'Package'}</span></td>
                      <td style={td}><b>{x.name}</b>{x.sampleTypes?.length > 0 && <div style={sub}>{x.sampleTypes.join(', ')}</div>}</td>
                      <td style={td}>₹{(x.mrp || x.price).toLocaleString('en-IN')}</td>
                      <td style={{ ...td, fontWeight: 900, color: '#087f6f' }}>₹{x.price.toLocaleString('en-IN')}</td>
                      <td style={td}>{x.tat || '—'}</td>
                      <td style={td}>{x.fastingNeeded ? 'Yes' : 'No'}</td>
                      <td style={td}>{x.active ? <span style={activePill}>Active</span> : <span style={inactivePill}>Deleted</span>}</td>
                      <td style={td}>
                        <div style={{ display: 'flex', gap: 6 }}>
                          <button onClick={() => edit(x)} style={smallBlue}>Edit</button>
                          {x.active ? <button onClick={() => remove(x)} style={smallRed}>Delete</button> : <button onClick={() => restore(x)} style={smallGreen}>Restore</button>}
                        </div>
                      </td>
                    </tr>
                  ))}
