...
type StoredBill={fileName:string;fileData:string;uploadedAt:string};
type ManualMeta={
  ...
  reportDocuments?:StoredReport[];originalBill?:StoredBill;
};
...
export async function GET(request:Request){
 ...
 return NextResponse.json({orders:rows.map(b=>{
   ...
   const reportDocs=m?.reportDocuments||[];const reportType=reportDocs.some(x=>x.reportType==='FULL')?'FULL':(m?.reportType||'FULL');const hasReport=reportDocs.length>0||!!b.reportData;
   return {id:b.id,orderId:b.id,billNumber:m?.billNumber||'',createdAt:sampleEntryAt,sampleEntryAt,patient:{name:p.name,age:p.age,gender:p.gender,phone:p.phone,email:p.email||m?.email||''},email:p.email||m?.email||'',doctorName:m?.doctorName||b.doctorName||'',tests:m?.tests||[],testAmount,homeCollectionCharge:charge,totalAmount:total,discount,paidAmount,balance,netAmount:net,paymentStatus:b.paymentStatus,paymentMode:storedMode,paymentModes:m?.paymentModes||[],sampleBarcodes:m?.sampleBarcodes||[],extraBarcodes:m?.extraBarcodes||[],sampleTracking:m?.sampleTracking||[],barcodeUpdatedAt:m?.barcodeUpdatedAt||null,sampleStatus,sampleStatusUpdatedAt:m?.sampleStatusUpdatedAt||null,sampleCollectedAt:b.sampleCollectedAt,sampleReceivedAt:b.sampleReceivedAt,processingStartedAt:b.processingStartedAt,reportName:b.reportName,reportType,reportAvailable:hasReport,reportReady:hasReport&&reportType==='FULL',reportReadyAt:b.reportReadyAt,reportDocuments:reportDocs.map(({fileData,...x})=>x),originalBillAvailable:!!m?.originalBill?.fileData,originalBillName:m?.originalBill?.fileName||null};
 })});
}
