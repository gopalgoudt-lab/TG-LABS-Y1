...
export async function PATCH(request:Request){
 try{
  ...
  await writeAdminAudit(request,{action:'THYROCARE_SAMPLE_TRACKING_UPDATED',entityType:'Booking',entityId:b.id,summary:`${identity.role} updated sample tracking for ${b.id}`,metadata:{role:identity.role,sampleStatus,missingRequiredSamples:missing,sampleTracking:tracking}});
  return NextResponse.json({ok:true,sampleStatus,sampleTracking:tracking,missingRequiredSamples:missing});
