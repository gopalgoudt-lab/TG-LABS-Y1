...
const editSchema=z.object({
 ...
 sampleBarcodes:z.array(sampleBarcodeSchema).max(20).optional().default([]),extraBarcodes:z.array(barcodeSchema).max(20).optional().default([]),sampleStatus:sampleStatusSchema.optional()
});
...
export async function PATCH(request:Request){
 try{
  ...
  await prisma.booking.update({where:{id:b.id},data:{doctorName:b.doctorName||null,totalAmount:net,paymentStatus:balance===0?'PAID':'PENDING',paymentMode:dbPaymentMode(b.paymentMode),paidAt:balance===0?(booking.paidAt||new Date()):null,workflowStatus,sampleCollectedAt:sampleStatus==='SAMPLE_COLLECTED'?(booking.sampleCollectedAt||now):booking.sampleCollectedAt,sampleReceivedAt:sampleStatus==='RECEIVED_AT_LAB'?(booking.sampleReceivedAt||now):booking.sampleReceivedAt,processingStartedAt:sampleStatus==='PROCESSING'?(booking.processingStartedAt||now):booking.processingStartedAt,reportReadyAt:sampleStatus==='REPORT_READY'?(booking.reportReadyAt||now):booking.reportReadyAt,adminNotes:JSON.stringify(meta)}});
  ...
  return NextResponse.json({ok:true,sampleStatus});
 }catch(error){
