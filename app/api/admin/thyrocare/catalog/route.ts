...
const itemSchema = z.object({
  type: z.enum(['TEST', 'PACKAGE', 'PROFILE']),
...
function slugify(name: string, type: CatalogType) {
...
function storedPackageType(slug: string): 'PACKAGE' | 'PROFILE' {
  return slug.startsWith('thyrocare-profile-') ? 'PROFILE' : 'PACKAGE';
}

async function resolveIncludedTestIds(selectedIds: string[], enteredNames: string[], parentType: 'PACKAGE' | 'PROFILE') {
  ...
  for (const name of uniqueNames) {
    ...
    if (!test) {
      test = await prisma.diagnosticTest.create({
        data: {
          slug: slugify(name, 'TEST'), name,
          description: `Added while creating a Thyrocare ${parentType.toLowerCase()}. Update price and test details if required.`,
          mrp: 0, price: 0, diagnosticPartner: PARTNER, fastingNeeded: false, sampleTypes: [], active: true,
        },
        select: { id: true },
      });
    }
...
async function replaceIncludedTests(packageId: string, testIds: string[]) {
...
export async function GET(request: Request) {
  ...
  const [tests, storedPackages] = await Promise.all([
  ...
  const packageLike = storedPackages.map((x) => ({
    ...x,
    type: storedPackageType(x.slug),
    includedTests: x.tests.map((row) => ({ id: row.test.id, name: row.test.name, active: row.test.active })).sort((a, b) => a.name.localeCompare(b.name)),
  }));
  return NextResponse.json({
    tests: tests.map((x) => ({ ...x, type: 'TEST' as const })),
    packages: packageLike,
    profiles: packageLike.filter((x) => x.type === 'PROFILE'),
  });
}
...
export async function POST(request: Request) {
  try {
    ...
    let testCount = 0;
    if (b.type !== 'TEST') {
      const testIds = await resolveIncludedTestIds(b.includedTestIds, b.customTestNames, b.type);
      await replaceIncludedTests(created.id, testIds);
...
export async function PATCH(request: Request) {
  try {
    ...
    if (b.type !== 'TEST') {
      const testIds = await resolveIncludedTestIds(b.includedTestIds, b.customTestNames, b.type);
      await replaceIncludedTests(id, testIds);
      testCount = testIds.length;
    }
...
export async function DELETE(request: Request) {
  try {
    await requireThyrocareRole(request,['ADMIN']); const body = await request.json(); const id = z.string().min(1).parse(body.id); const type = z.enum(['TEST', 'PACKAGE', 'PROFILE']).parse(body.type);
