...
const schema=z.object({
 name:z.string().trim().min(2).max(120),phone:z.string().regex(/^[0-9]{10}$/),email:z.string().trim().email().max(200),age:z.coerce.number().int().min(0).max(120).nullable().optional(),gender:z.enum(['Male','Female','Others']).nullable().optional(),mode:z.enum(['HOME','CENTRE']),address:z.string().trim().max(500).optional().default(''),pincode:z.string().regex(/^[1-9][0-9]{5}$/).optional().default(''),date:z.string().date(),slot:z.string().trim().min(3).max(60),testIds:z.array(z.string()).default([]),packageIds:z.array(z.string()).default([]),technician:z.string().trim().max(120).optional().default(''),totalAmount:z.coerce.number().int().min(0),status:z.enum(['PENDING','CONFIRMED','CANCELLED','COMPLETED']),paymentStatus:z.enum(['PENDING','PAID','FAILED','REFUNDED']),paymentMode:z.enum(['ONLINE','CASH','CARD','UPI','BANK_TRANSFER','OTHER']).nullable().optional(),paymentTiming:z.enum(['ONLINE_NOW','SAMPLE_COLLECTION']).optional().default('ONLINE_NOW'),workflowStatus:z.enum(WORKFLOW),adminNotes:z.string().trim().max(1000).optional().default(''),reportName:z.string().trim().max(255).optional().default(''),reportData:z.string().max(4500000).optional().default('')
...
export async function PATCH(request:Request,{params}:{params:Promise<{id:string}>}){
 try{
  ...
  if(b.paymentStatus==='PAID'&&existing.paymentStatus!=='PAID')stamps.paidAt=now;
  if(b.paymentStatus!=='PAID')stamps.paidAt=null;
  ...
  const booking=await prisma.$transaction(async tx=>{
   ...
   return tx.booking.update({where:{id},data:{patientId,mode:b.mode,address:b.mode==='HOME'?b.address:null,pincode:b.mode==='HOME'?b.pincode:null,collectionDate:new Date(`${b.date}T00:00:00.000Z`),slot:b.slot,technician:b.technician||null,totalAmount:b.totalAmount,status:autoStatus,paymentStatus:b.paymentStatus,paymentMode:b.paymentMode??null,paymentTiming:b.paymentTiming,workflowStatus:b.workflowStatus,...stamps,adminNotes:b.adminNotes||null,reportName:b.reportName||null,reportData:b.reportData||null,items:{create:[...testMap.values()].map(t=>({testId:t.id,price:t.price}))}},include:{patient:true,items:{include:{test:true}}}})
  });
  return NextResponse.json({booking});
