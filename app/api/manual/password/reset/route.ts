...
export async function POST(request:Request){
 try{
  ...
  if(verified.databasePhone!==thyrocareLoginPhone())return NextResponse.json({error:'This mobile number is not authorized to reset Thyrocare passwords.'},{status:403});
  await saveThyrocarePassword(verified.databasePhone,role,newPassword);
  try{await prisma.adminAuditLog.create({data:{adminPhone:verified.databasePhone,action:'THYROCARE_PASSWORD_RESET_OTP',entityType:'THYROCARE_SESSION',summary:`${role} Thyrocare password reset after Firebase OTP verification.`,ipAddress:(request.headers.get('x-forwarded-for')||'').split(',')[0].trim()||null,userAgent:request.headers.get('user-agent')||null}})}catch{}
