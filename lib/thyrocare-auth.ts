...
export function thyrocareLoginPhone(){return normalizeIndianDatabasePhone(process.env.THYROCARE_LOGIN_MOBILE||'')}
...
export async function issueThyrocareSession(phone:string,role:ThyrocareRole){
 ...
 const payload=textToB64url(JSON.stringify({iss:'tg-labs-thyrocare',aud:'tg-labs-thyrocare-manual',phone:normalizeIndianDatabasePhone(phone),role,iat:now,exp:now+60*60*10}));
...
export async function verifyThyrocareSessionToken(token:string):Promise<ThyrocareIdentity>{
 ...
 if(typeof p.exp!=='number'||p.exp*1000<Date.now()||normalizeIndianDatabasePhone(p.phone)!==thyrocareLoginPhone())throw new Error('UNAUTHENTICATED');
 ...
 return {phone:normalizeIndianDatabasePhone(p.phone),role:p.role,exp:p.exp};
}
