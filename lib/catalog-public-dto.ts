...
type Product = { id: string; slug: string; name: string; aliases?: string[]; description?: string | null; preparation?: string | null; fastingNeeded: boolean; fastingHours?: number | null; parameterCount?: number | null; sampleTypes: string[]; categories?: Array<{ category: { slug: string; name: string; description?: string | null } }>; partnerOffers?: Offer[]; tests?: Array<{ test: { slug: string; name: string; fastingNeeded?: boolean; sampleTypes?: string[] } }> };

function baseProduct(value: Product, type: 'TEST' | 'PROFILE' | 'PACKAGE') {
  return { id: value.id, type, slug: value.slug, name: value.name, aliases: value.aliases ?? [], description: value.description ?? null, fastingNeeded: value.fastingNeeded, fastingHours: value.fastingHours ?? null, parameterCount: value.parameterCount ?? null, sampleTypes: value.sampleTypes, categories: (value.categories ?? []).map((x) => categoryDto(x.category)), offers: (value.partnerOffers ?? []).map(partnerOfferDto) };
}
