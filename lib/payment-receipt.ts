...
export async function createPaymentReceiptPdf(data:PaymentReceiptData):Promise<Uint8Array>{
 ...
 // Header: deliberately typeset as a lock-up so branding cannot collide.
 t('TG',38,785,31,bold,navy); t('LABS',86,785,31,bold,teal); t('DIAGNOSTICS  •  HOME COLLECTION',40,770,8,bold,navy);
 t('TRUSTED LABS',366,793,8,bold,navy); t('HOME COLLECTION',430,793,8,bold,navy); t('BETTER TOMORROW',500,793,8,bold,navy);
 line(350,768,555,768,border,.7);
 ...
 // Summary cards are anchored in the lower half for a clean portrait layout.
